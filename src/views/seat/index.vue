  ...
  <div id="">
    ...
    <van-list>
      <van-swipe-cell :before-close="beforeClose" :index="index" :id="item.id" v-for="(item,index) in items" :key="index">
        <van-cell :border="false" :title="item.name" :value="item.status == 1 ? '未使用' : '正在使用'" @click="updateHandle(item)" />
        <template #right>
...
<script>
import { getSeats, addSeat, updateSeat, deleteSeat } from "@/api/seat";
export default {
  name: "",
  data() {
    return {
  ...
  methods: {
    ...
    async updateData() {
      ...
      try {
        await updateSeat(this.formData);
        this.$toast("更新成功");
